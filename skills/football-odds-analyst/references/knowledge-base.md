...
> **v3.7.0 后**: 平局专项的判定已迁至 KB-17.3 平局信号分级体系（一级/二级信号）。KB-5b 的置信度调整保持有效，作为 KB-17.3 的补充参考。
...
## KB-16: 市场共识方向分箱准确率 — 179场跨赛事校准 (v3.9.0, v3.10.5更新)

> **v3.10.5**: 方向锚点从 Pinnacle 独家切换为 30 家博彩公司收盘赔率平均值 (百家平均)。历史校准基于 Pinnacle 数据，但 2022WC 验证 Pinnacle 与市场均值完全一致 (64场无偏离)，分箱表无需重校。
> **v3.9.0 跨赛事重校准**: WC2022(64) + EURO2024(51) + WC2018(64) = 179场。
> 废除 63.6% 单一基准（来自WC2026 44场偏估值）。替代为 54.7% 全量基线 + 7区间分箱表。
> 核心发现: 共识方向准确率在不同赔率区间差异极大——1.21-1.35区间达85.7%, 1.36-1.50区间仅40%。

### 16.1 共识方向分箱准确率表 (179场, 3赛事)
...
#### 17.2.4 五规则 (使用可计算输入, v3.10.7 新增规则2b)
...
规则2 (动机流失 — v3.10.7 取消赔率门槛):
  Pin方向方 mot < 0.25 → 按赔率分档处理
  背景: mot<0.25 只在 R3 锁定出线/已淘汰场景出现 (KB-17.2.2). 2026WC 回测发现赔率 1.50-2.00 区间同等危险 (美国1.89/德国1.63两场100%反转).
  注意: 看 Pin方向方的 mot, 不是主队的 mot

  子规则2a (深盘动机流失, 赔率<1.50):
    Pin方向方 mot < 0.25 AND 赔率 < 1.50 → draw risk VERY HIGH
    动作: 强制"跳过"
    若对手赔率 ≥ 3.0 AND KB-17b 反投条件 1-4 全满足 → 可选反投对手单关 ¥10
    二层验证 (v3.8.2): 
      若竞彩赔率显著低于 Pinnacle (价差>10%) → 不跳过, 降为"候选"(0.5x), 标记"动机待确认"

  子规则2b (中等价位动机流失, 赔率1.50-2.00 — v3.10.7新增):
    R3 AND Pin方向方 mot < 0.25 AND 赔率 ∈ [1.50, 2.00] → 轮换风险
    数据来源: 2026WC D/E/F末轮, 美国1.89和德国1.63两场锁定出线后100%反转
    动作: 降为"C类", 不入串关核心, 不入"胆"
    不为反投策略打开 (赔率不够高, 反投不具正EV)
    金额限制: C类仓位上限 ¥15 (单关), 不参与串关
...
> **v3.8.0**: 废除零散手工提取，替代为从6页全量JSON计算的11个聚合信号。
