...
三级响应（v3.3.1 回测校准）:
  0-1 分: 正常，不调整
  2 分:   ⚠️ 衰减   conf × 0.85, 仓位 × 0.70
  3 分:   ⚠️ 衰减 + DRM联动检查: 若 DRM≥1 → 升为 🔴 严重
  4-5 分: 🔴 严重   conf × 0.70, 禁入串关核心，仅可容错腿或单关 ≤1%
  6 分+:  🚫 熔断   方向保留（估值用），投注操作停止

> 校准依据: 2022世界杯64场回测。FVS=3 时命中率 60%（仍高于基线），但叠加 DRM≥1 后净收益 +5。
> FVS 在正常市场中呈"共识越强命中率越高"趋势（FVS=0:44%, FVS=1+:64%+）；
> FVS-2/FVS-3（全低赔日/中赔扎堆）在极端市场条件（如2026-06-22）下转为警告信号。
```
...
### 17.3 响应等级（v3.3.1 回测校准）

> 校准依据: 2022世界杯64场回测。DRM≥2 时热门命中率从63%断崖式跌至22%，净收益+9（16次挽救−7次误伤）。
> DRM=1 反而提升命中率至71%（轻微平局倾向帮热门过滤了劣质信号）。
> DRM≥2 触发时，**所有方向性 SPF 投注视为高风险**。

```
DRM 总分    响应                                对 MBI 方向的影响
─────────────────────────────────────────────────────
0-1 分    低平局风险，不调整                       方向置信度不变
2 分+     🔴 VETO — 禁止该场方向性 SPF 投注     方向置信度 ×0.70
          仅可大小球/双方进球替代玩法             deVig(平) floor=28%
          parlay_eligible = False              输出标注"🔴 平局风险: SPF不可用"
```

> v3.3.0 原设计（二级响应 2-3→⚠️/4-5→🔶/6+→🚩）过于复杂且区分度低。
> 回测证明 DRM 的核心价值在 2 分阈值：DRM=2 的 18 场中 12 场热门失败（其中9场平局），
> DRM≥3 仅有 5 场样本不足以独立得出结论。

### 17.4 FVS × DRM 叠加矩阵（v3.3.1 回测校准）

```
v3.3.1 简化: DRM 仅 0-1(正常) 和 2+(VETO) 两档。

              DRM 0-1 (正常)     DRM 2+ (VETO)
FVS 0-1    ✅ 正常 SPF          🔴 平局风险 → 换玩法
FVS 2      ⚠️ 衰减仓位          🔴🚫 双重警告 → 跳过 SPF
FVS 3      ⚠️ 衰减+DRM联动      🔴🚫 跳过此场
FVS 4-5    🔴 热门高危           🚫 跳过此场
FVS 6+     🚫 热门熔断           🚫 跳过此场

实战含义:
  ✅ 正常 SPF: 可入选串关核心，正常仓位
  ⚠️ 衰减仓位: 仓位×0.70，可入选容错腿
  🔴 平局风险: 该场 SPF 方向禁用，换大小球/双方进球
  🔴🚫: 整场跳过 SPF，仅可单关极轻仓(≤0.5%本金)
  🚫: 该场仅估值用，完全不投
```

> 校准依据: 2022世界杯回测。DRM≥2 时热门命中率仅22%，FVS=4-5 仅3场样本（全对但样本太少）。
> 原矩阵 4×4=16格 → 简化为 5×2=10格，大幅降低决策复杂度。
