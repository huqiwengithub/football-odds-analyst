...
对一场新比赛:
  ...
  步骤B: 对每个OCI, 在179场历史库中查找:
    ...
    - 取这些场次的平均命中率 = 该指标原始命中率
    - 归一化权重 = (原始命中率 - 0.50) × 2
      (归一化后: +0.28表示强正向, +0.10表示弱正向,
       -0.06表示弱负向, -0.14表示强负向)
  步骤C: 5个归一化权重加权平均 = 综合信心分(0~1)
    正值(>0) → 正向确认系统方向
    负值(<0) → 系统方向可靠性降低
  步骤D: 综合信心分 → 仓位档位
```

### 16.4 179场实测权重表（v3.5.3 校准）

> **v3.5.3 更新**: OCI4=CONVERGE 升级为核心仓位触发条件(2026WC验证5/5=100%)。
> OCI4=DIVERGE 禁止入核心仓位(12场中6/6=50%=噪声)。详见 betting-sop.md 三档仓位。

```
以下权重来自179场跨赛事回测(2022WC 64场 + 2018WC 64场 + 2024Euro 51场):

┌──────────────────────────┬──────┬──────┬────────┬────────────┐
│ OCI模式                  │ 命中 │ 总数 │ 原始率  │ 归一化权重  │
├──────────────────────────┼──────┼──────┼────────┼────────────┤
│ OCI1_FLAT   (热门持平)    │  25  │  39  │ 64.1%  │ +0.28 │ 🟢  │
│ OCI5_INDEP  (AH独立)     │  14  │  22  │ 63.6%  │ +0.27 │ 🟢  │
│ OCI4_CONV   (离散收敛)    │  77  │ 129  │ 59.7%  │ +0.19 │ 🟢  │
│ OCI5_SYNC   (盘口同步)    │   4  │   7  │ 57.1%  │ +0.14 │ 🟡  │
│ OCI1_DROP   (热门降水)    │  43  │  76  │ 56.6%  │ +0.13 │ 🟡  │
│ OCI2_AGREE  (市场共识)    │  97  │ 177  │ 54.8%  │ +0.10 │ 🟡  │
│ OCI2_DISAG  (市场分歧)    │   1  │   2  │ 50.0%  │ +0.00 │ ⚪  │
│ OCI1_RISE   (热门升水)    │  30  │  64  │ 46.9%  │ -0.06 │ 🔴  │
│ OCI4_NEUT   (离散中性)    │  21  │  49  │ 42.9%  │ -0.14 │ 🔴  │
│ OCI4_DIV    (离散发散)    │   0  │   1  │  0.0%  │ -1.00 │ 🔴  │
└──────────────────────────┴──────┴──────┴────────┴────────────┘

解释:
  🟢 强正向(+0.15以上): OCI1_FLAT | OCI5_INDEP | OCI4_CONV
      → 这些模式出现时，系统方向判定有显著增量确认
  🟡 弱正向(+0.05~+0.15): OCI5_SYNC | OCI1_DROP | OCI2_AGREE
      → 信息增量有限，权重应低于强正向
  🔴 负向: OCI1_RISE(-0.06) | OCI4_NEUT(-0.14)
      → 这些模式是持续低于随机(46.9%/42.9%)的反向指标，应降低系统方向信心
      → 注意: OCI4_NEUTRAL的负向影响二倍于OCI1_RISE
  ⚪ 样本不足: OCI2_DISAG | OCI4_DIV → 降权50%

权重限制规则:
  - 任何一个指标的归一化权重的绝对值不能超过0.30
  - 少于3场历史匹配的模式 → 标记为"样本不足" → 权重×0.50
  - 权重累加时，总浮动的上限为±0.30（防止单个指标过度主导）
```
...
```
每次回测后自动更新上述权重表:
  - 历史库每增加50场回测数据 → 重新计算所有模式的命中率和归一化权重
  - 更新时保留所有历史数据，不做丢弃
  - 新权重覆盖旧权重，立即生效

---

## KB-17: DRM — Draw Risk Module 
...
> **v3.5.2 更新**: 废除STANDARD档,仓位简化为三档(核心/跳过/冷门翻转)。
