...
## KB-17: 平局专项模型 (v3.7.0 校准重写)

> **v3.7.0**: Pinnacle 44场中0次最低赔率是平局。13场平局全部来自"强队对弱队爆冷平"。
> 平局必须独立于赔率方向建模。废除原有DRM评分体系，替代为两阶段检查。

### 17.1 平局信号检查 (5条)

```
以下条件各自独立命中率待校准，目前保留作为标记而非分数:
...
触发≥3条 → 方向标记为"跳过"(非"D")
原因: 赔率方向无法预测平局，标记为跳过比强行输出"D"更安全
```

### 17.2 废除的DRM规则

以下从v3.7.0起删除:
- DRM-2: OU线检测 → 移至进球倾向(Step 10)
- DRM-3: 近期平局惯性 → 样本不足, 无统计支持
- DRM-7: 淘汰赛+2 → 主观标签
- DRM打分体系 → 被"触发计数"替代
