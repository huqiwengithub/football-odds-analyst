...
### Step 10.5 — OCI 客观指标分析 (KB-16)
- **v3.5.0 重构**: 13条静态FVS规则替换为5个OCI客观变化指标
- **必须先读 KB-16**
- 5 维度并行测量（OCI-1 至 OCI-5），全部基于盘口可观测量化变化:
  - OCI-1: Pinnacle赔率变轨（开盘→收盘方向+幅度）
  - OCI-2: 市场共识变轨（30家均值方向 vs Pinnacle）
  - OCI-3: 成交量结构（量价同步/背离）
  - OCI-4: 机构离散度（标准差6h变化）
  - OCI-5: AH/OU线验证（盘口分裂检测）
- 权重获取: 每个OCI查179场历史库中相同变化模式的命中率
- OCI 不改 MBI 方向，改置信度、仓位和串关资格
- 输出：5个OCI各自权重 + 综合信心分

### Step 10.6 — DRM 平局因子 (KB-17)
- **v3.5.0 清理**: 废除DRM-7/9/10, DRM改为OCI补充因子
- **必须先读 KB-17**
- DRM 不再独立计分，作为 OCI-4(离散度)和 OCI-5(AH/OU)的补充:
  - DRM-1: deVig(平)检测 — >28%仓位×0.80, >32%换玩法
  - DRM-2: OU线变化测量 — OU线下调>0.25时注意平局
  - DRM-3: 近期平局惯性 — 任一方近5场≥2平
- 输出：DRM 补充因子权重 + 对OCI综合分的调整值

### Step 10.7 — 四档仓位引擎 (KB-18)
- **v3.5.0 更新**: 仓位由OCI加权综合信心分驱动，新增冷门翻转档
- **必须先读 KB-18**
- 5个OCI权重加权平均 → 综合信心分 → 匹配四档:
  - 🔥 **核心仓位**（信心分≥65%）：全仓，入串关核心
  - ✅ **标准仓位**（信心分50-65%）：半仓，入容错腿
  - 🚫 **跳过**（信心分<50%+偏离<2）：不投
  - 🔄 **冷门翻转**（信心分<50%+偏离≥2）：押冷门/平局，仅单关
- 输出：每场档位归属 + 仓位乘数 + 偏离计数 + 建议投注金额

### Step 10.8 — 偏离检测 + 冷门翻转触发 (KB-19)
- **v3.5.0 重写**: 5条客观偏离信号，废除赛事类型感知
- **必须先读 KB-19**
- 5条偏离信号并行检测:
  - 信号1: Pinnacle升水>3%（热门赔率在上升）
  - 信号2: 量价背离（成交增+赔率不降）
  - 信号3: 市场分歧（30家均值与Pinnacle方向相反）
  - 信号4: 平局偏高（deVig(平)>28%）
  - 信号5: OU线下调>0.25（进球预期降低）
- 偏离计数≥2 → 冷门翻转候选 | ≥3 → 确认翻转
- 输出：偏离信号清单 + 计数 + 翻转方向（平局/逆转）

### Step 11 — 组合构建 + 三档仓位执行
- **⚠️ 必须先读 `references/betting-sop.md` 完整五步 SOP，不可仅凭下方速览执行**
- **v3.3.0 前置检查**: 逐条腿计算竞彩 EV（KB-13.8），EV<−0.05 → 不得入选串关
- **v3.5.0 仓位前置**: 先执行 Step 10.7 四档仓位引擎，获取每场档位归属
  - 🔥 核心仓位: 可入串关核心，全仓
  - ✅ 标准仓位: 可入容错腿，半仓
  - 🚫 跳过: 不投
  - 🔄 冷门翻转: 仅单关
...
□ Step 10.5 OCI 5维度客观指标已分析 → 输出 OCI 权重 + 综合信心分
□ Step 10.6 DRM 平局因子已补充 → OCI 加权已含平局调整
□ Step 10.7 四档仓位已执行 → 每场档位归属 + 偏离计数
□ Step 10.8 偏离检测已执行 → 偏离信号清单 + 冷门翻转判断
...
客观指标: OCI 综合信心分 X% [🔥核心/✅标准/🚫跳过/🔄冷门翻转] — 偏离信号: [计数]
