...
- **v3.5.0 仓位前置**: 先执行 Step 10.6 四档仓位引擎，获取每场档位归属
  - 🔥 核心仓位: 可入串关核心，全仓
  - ✅ 标准仓位: 可入容错腿，半仓
  - 🚫 跳过: 不投
  - 🔄 冷门翻转: 仅单关
...
### 术语翻译
- 完整翻译表见 `references/term-translation.md`
- 核心规则: 报告不允许出现任何英文缩写。内部计算保留，输出必须翻译为中文。
...
| **16** | **OCI 客观指标+DRM平局因子: 5维度变化测量, 历史权重查表** | **10.5** |
| **17** | **DRM 平局因子: deVig+OU+近期平局, OCI补充因子, 已废除DRM-7/9/10** | **10.5** |
| **18** | **四档仓位: 核心/标准/跳过/冷门翻转, OCI加权分驱动** | **10.6** |
| **19** | **偏离检测: 5条信号, 冷门翻转触发条件** | **10.6** |
