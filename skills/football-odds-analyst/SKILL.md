---
name: football-odds-analyst
description: "Football odds analyst v3.10.7 — 规则2无赔率门槛+伤病logit按角色分层+锁定出线降C类. 179场校准."
allowed-tools: Read, Write, Bash, WebSearch, WebFetch
agent_created: true
version: "3.10.8"
released: 2026-06-26
references: references/knowledge-base.md, references/betting-sop.md, references/fundamentals/
dependencies:
  - name: 500com-football-scraper
    required: true
    install_from: marketplace
    description: "500.com deep data scraper — provides per-match 6-page deep analysis JSON + sporttery.cn fallback for 竞彩赔率"
---

# Football Odds Analyst v3.10.8 — 市场共识方向 + 概率驱动哑铃组合 + 全6类基本面 + 分界墙

> **⚠️ 三大铁律**:
> 1. **分析用全球数据** (Steps 1-10.5): 使用 30 家博彩公司平均值 (百家平均) + Pinnacle + 必发成交量做盘口分析
> 2. **投注用竞彩官网赔率** (Step 11): 主源 sporttery.cn 页面抓取 (scripts/fetch_sporttery.py); 后备 500.com ouzhi/rangqiu 页"竞*官*"行
> 3. **全报告基于主队视角**: 所有投注建议、方向描述都以主队为参照系

> **文件分工**：
> - `references/knowledge-base.md`（KB-0~KB-19）→ **盘口分析**：赔率解读、陷阱检测、MBI、OCI客观指标、DRM平局因子、偏离检测
> - `references/betting-sop.md` → **投注执行**：五步SOP、M串N动态仓位分配、熔断空仓铁律、信号分级、EV执行、仓位规则、单关检查、竞彩赔率提取
> - `football-backtest-workflow/` → **复盘回测**：赛后 7 阶段/9 层系统化复盘，替代旧版 postmortem.md
> - `500com-football-scraper/` → **数据源**：500.com 深度数据抓取与缓存

---

## ⚠️ 执行纪律（执行任何分析前必须完成）

```
┌─────────────────────────────────────────────────────────────┐
│                    STOP — 先做完以下 3 件事                     │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. 阅读 KB-2（欧亚陷阱）+ KB-4（6D 评分）                      │
│     Read references/knowledge-base.md → 定位 KB-2 和 KB-4    │
│                                                             │
│  2. 阅读相关 KB（按步骤需要）                                    │
│     每步执行前，先打开对应 KB 章节，不靠记忆                       │
│                                                             │
│  3. 13 步 + Step 13（赛后回测触发）必须全部执行，禁止跳过          │
│     即使某步看起来"简单"（如 Step 1.5），也必须显式输出结果          │
│                                                             │
│  熔断检查（每步后自查）:                                         │
│    单日亏 3%→停止 / 周亏 8%→转入影子测试 / 模块 7 连错→禁用       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Step 0: Data Pipeline — 按比赛 ID 查缓存，按需抓取

> ⚠️ **先取比赛 ID，按 ID 查缓存，再按需抓取**。所有足彩技能共用同一共享缓存。

```
1. 调用 500com-football-scraper Phase 0 Step 0a → 获取 match_list（含 shuju_id）
   （通过 Liansai API 或 trade 页面，轻量请求）
2. 对 match_list 中的每场比赛 shuju_id:
   检查 .cache/shared-football/parsed/{date}_{shuju_id}.json
   → 存在且 mtime < 1h: 标记 CACHED，直接读 JSON
   → 不存在或过期: 标记 NEED_FETCH，传给 scraper
3. 触发 500com-football-scraper Phase 0 Step 0c
   → 仅对 NEED_FETCH 的比赛逐场逐页抓取
   → 每场比赛独立保存到 .cache/shared-football/parsed/{date}_{shuju_id}.json
4. 加载所有比赛的 JSON → 进入 Step 1
```

> ⚠️ **不再搜索 `{date}_deep.json`**。每场比赛按自己的 shuju_id 独立缓存，
> 不同比赛不互相影响。一场数据过时不影响其他场。

所需字段：`ouzhi`（pinnacle + all_bookmakers + dispersion）、`yazhi`、`shuju`、`touzhu`、`basic`。

### Step 0a — 加载基本面数据 (v3.9.0 新增)

```
1. 加载 references/fundamentals/team_strength.json:
   对每场比赛的两队:
     查 ELO 评分 → 计算 ELO 差距 → 查表得预期净胜球+冷门概率
     查 FIFA 排名 → 计算排名差距
     查实力档位 → S/A/B/C/D/E

2. 获取小组积分榜 (⚠️ P0 强制 — 有小组赛的赛事必做):
   WebFetch https://liansai.500.com/zuqiu-{sid}/jifen-{jid}/
   遵循 references/fundamentals/standings-protocol.md v2.0:
     a. 提取全部小组 standings (P/W/D/L/GF/GA/GD/pts/rank)
     b. 四道数学验证: W+D+L=P / GF-GA=GD / pts=W×3+D / 同组P差≤1
     c. WebSearch 交叉验证: "[赛事] 小组积分榜" 对照
        → ≥80% 一致 → STANDINGS_VERIFIED
        → <80% 一致 → STANDINGS_DISCREPANCY, 人工检查
        → 四验证任一失败 → STANDINGS_INVALID, 禁止 mot 计算
     d. 输出验证报告 (数据源/四验证/交叉验证/状态/时间戳)

3. 获取形态+H2H数据 (v3.9.4 新增):
   对每场候选比赛:
     WebFetch https://odds.500.com/fenxi/shuju-{shuju_id}.shtml
     遵循 references/fundamentals/form-h2h-protocol.md:
       a. 近期形态: 近10场 total/home/away W-D-L/GF/GA → form_score
       b. 交锋历史: 近N次 H2H → h2h_score (时效加权)
       c. 预计阵容: 伤病/停赛标注
     量化输出: form_score + h2h_score → 传入 Step 2

4. 新闻搜索 (Step 1.5 阶段执行):
   遵循 references/fundamentals/news-protocol.md 白名单/黑名单
```

> ⚠️ **【必读】竞彩赔率数据源⚠️**
> **分层使用原则**:
> - **分析层 (Steps 1-10.5)**: 使用 Pinnacle + 30 家博彩公司的全球数据（来自 ouzhi/yazhi/shuju/touzhu 页面）做盘口分析、MBI
> - **执行层 (Step 11)**: 使用竞彩官方赔率做投注建议和 EV 计算
>
> **竞彩官方赔率的正确提取 (v3.10.7 webapi 优先)**:
> - 🥇 **主源: sporttery.cn REST API** → `python3 scripts/fetch_sporttery.py --date {date} --json`
>   - 调用 https://webapi.sporttery.cn/gateway/uniform/football/getMatchCalculatorV1.qry?channel=tycp
>   - 解释: `m.sporttery.cn/mjc/jsq/zqspf/` 是 SPA 空壳页面，真实数据通过 AJAX 加载自 webapi REST API
>   - SPF 赔率: JSON had 对象的 h(主胜)/d(平局)/a(客胜)
>   - RQSPF 赔率: JSON hhad 对象的 goalLine(让球数)/h(让球主胜)/d(让球平)/a(让球客胜)
>   - 缓存: `~/.cache/workbuddy/sporttery/{date}_jingcai.json`, 1 小时过期
> - 🥈 **后备: 500.com** → scraper Phase 3 ouzhi 页 row 1 "竞*官*" (SPF) + rangqiu 页 row 1 "竞*官*" (RQSPF)
>   - 仅在 sporttery.cn API 失败时使用
> - **切勿**从 ouzhi 页面的"百家平均"列获取竞彩赔率
>
> **错误后果**：使用百家平均替代竞彩赔率，会导致赔率数量级错误（如1.92误为1.14），所有EV计算和串关方案全部失效。

---

## Step 0.5 — 市场健康检查 (KB-14)

```
⚠️ 必须先读 KB-14
对每场候选比赛计算:
  PVD_Score (价量背离) + CD_Score (渠道背离) → MPC
  如有多帧数据 → Pulsation F/D 矩阵 → TWQ

三级响应（永不翻转 MBI 方向判定）:
  CIRCUIT_BREAKER (MPC < −0.8 或 MPC<−0.5+TWQ异常)
    → 整场从候选池移除
  VETO (MPC < −0.5)
    → 该场禁止投注（仅估值用）
  NORMAL (|MPC| < 0.2 → 不修正; 否则信度 × (1+MPC×0.10))
```

## 14 步执行流程 (Step 0 → Step 0a → Step 0.5 → Step 1 → ... → Step 13)

> ⚠️ 每步必须输出结果。禁止"看起来简单就跳过"。每步标注要读取的 KB 章节。

### Step 1 — 加载数据 (Step 0.5 市场健康检查已完成)
- 加载 `.cache/shared-football/parsed/{date}_{shuju_id}.json` (独立缓存, 一场一文件)
- 队名校验（KB-0 协议）
- 确认 6 页完整（ouzhi / yazhi / rangqiu / daxiao / shuju / touzhu）
- 输出：`N 场比赛数据就绪，[队名列表]`

### Step 1.5 — 反叙事筛查 + 新闻搜索
- 新闻驱动？首秀/关键缺席？历史平局率 >27%？
- **v3.9.0 新增**: 执行 `references/fundamentals/news-protocol.md` 新闻搜索
  - 仅搜白名单场景 (伤病/首发/天气/突发事件)
  - 搜索结果必须与赔率变动时间戳交叉验证
  - 新闻信号只降信心不翻方向
- 参考 KB-5 平局基准线
- 输出：`[场次编号] 触发反叙事：[原因]` 或 `无反叙事信号`
  + 新闻信号面板: 伤病/突发事件/首发/赔率交叉

### Step 2 — 基本面量化分析 (v3.9.4 重构)

> **v3.9.4**: 全 6 类基本面补全。新增近期形态 + H2H 交锋 + 赛程体能独立层。
> 数据源: ELO/FIFA (team_strength.json) + 积分榜 (standings-protocol) + 形态/H2H (form-h2h-protocol via shuju页)。

```
基本面量化六维度:

1. ELO 实力对比 (team_strength.json):
   查两队 ELO → 计算差距 → 查表得:
     ELO差≥200  → 碾压级, 预期净胜球>2.5, 冷门<8%
     ELO差100-200 → 明显差距, 预期净胜球1.0-2.5, 冷门8-15%
     ELO差50-100  → 中等差距, 预期净胜球0.5-1.0, 冷门15-25%
     ELO差0-50    → 接近, 预期净胜球0-0.5, 冷门25-40%

2. 近期形态 (form-h2h-protocol.md, shuju页抓取):
   form_score = (W×3+D×1)/(P×3), 近10场 + 主客分拆
   形态方向: ≥0.70→强势 / 0.40-0.70→一般 / <0.40→低迷
   主客分裂: |home_score−away_score|>0.30→标记
   形态×ELO交互: 逆势/悖论/共振 → 仓位调整

3. 交锋历史 (form-h2h-protocol.md, shuju页抓取):
   时效窗口: <5年→全权重 / 5-10年→0.5x / >10年→0.2x
   h2h_score = (W+0.5×D)/P (时效期内)
   无时效期内交锋 → null (无参考价值, 不影响判定)

4. 伤病/停赛 (news-protocol.md 场景 1, v3.10.7 按角色分层):
   按角色分级 (news-protocol.md 规则2 详细表):
     核心持球点/组织核心缺阵 → logit -0.25
     核心射手/防线核心/门将缺阵 → logit -0.20
     角色球员缺阵 → logit -0.10
     轮换球员 → 不计入
   双核同时缺阵 → logit -0.35 | 防线重组(2+人) → 对方进攻 +0.10

5. 赛程体能 (Liansai API 比赛日期间隔):
   休息 <3 天 → 体能紧张 | 休息 3-4 天 → 一般 | 休息 ≥5 天 → 充分
   下一场重要性: 如为淘汰赛→轮换意愿低/如无关紧要→轮换意愿高

6. 小组形势 (Liansai API 积分 + standings-protocol 验证):
   mot = base_situation(积分榜查表) + elo_adjust + rest_adjust (KB-17.2.2)

输出:
  [场次] 基本面量化:
    ELO差: [+XXX] → 预期净胜球:[X.X], 冷门概率:[XX%]
    实力档: [S-E] vs [S-E]
    近期形态: 主队 [XW-XD-XL] form:[X.XX→强势/一般/低迷]
               客队 [XW-XD-XL] form:[X.XX→强势/一般/低迷]
               [主客分裂标记] [形态×ELO交互标记]
    交锋: 近[X]次 主[X]胜[X]平客[X]胜, 时效:[有效/降权/过旧]
          H2H方向:[主队占优/均势/客队占优/无参考]
    伤病: [X人核心缺阵] → logit [-0.XX]
    赛程: 休息[X]天, 下一场在[X]天后 → 体能:[充分/一般/紧张]
    形势: [生死战/无关紧要/正常], mot:[X.XX]
    基本面倾向: [方向], 量化强度: [高/中/低]
      (强度规则: ELO差≥100+无核心缺阵+form≥0.50→高
                  ELO差50-99或form=0.30-0.49→中
                  ELO差<50或form<0.30→低)
```

### Step 3a — MBI 多机构共识（三方向并行）
- **必须先读 KB-10**
- 执行 SCS → DRI → Lead-Lag → WaterFlow → Exchange → Kelly
- **v3.2.0 新增**: Lead-Lag + Exchange 必须做**赔率×成交量交叉验证**（KB-10.3b 四象限矩阵）
  - 区分仓位驱动调价 vs 信息驱动调价 vs 庄家诱导
- **⚠️ v3.6.2 修正（2026-06-24 诊断驱动）**:
  - **三方向并行评分**，禁止只输出最低赔率方向
  - 对 `home` / `draw` / `away` 三个方向各自计算 MBI 信心分
  - 输出三方向 MBI 面板（每模块对三个方向的判定 + 量价交叉验证结论）
  - 方向原始输出格式改为:
    ```
    MBI 三方向评分:
      主胜: X.X 分 (SCS=.. DRI=.. LeadLag=.. Water=.. Exchange=.. Kelly=..)
      平局: X.X 分 (...)
      客胜: X.X 分 (...)
      推荐方向: [主胜/平局/客胜] (评分最高者)
    ```

### Step 3b — 概率数学
- **必须先读 KB-1**
- Shin de-vig 去抽水 + 层级均值 overround 计算
- 输出：Pinnacle 真实概率、市场隐含概率、抽水比例

> **🚨 RQSPF 坐标系铁律 (2026-06-26 事故修复)**: 
> 竞彩 RQSPF 的让球数始终作用于**主队**，三选项永远从主队出发:
> ```
> 实例: 新西兰(主) vs 比利时(客), RQSPF让2球
>   ├─ 主胜(让2球) @2.58 = 新西兰+2 > 比利时
>   │   实况: 比利时赢0-1球 / 平局 / 新西兰赢
>   ├─ 让2球平 @3.95    = 新西兰+2 = 比利时
>   │   实况: 比利时赢恰好2球 (如 0-2)
>   └─ 客胜(让2球) @2.05 = 新西兰+2 < 比利时  
>        实况: 比利时赢3球以上
> ```
> **三坐标映射验证** (每用 RQSPF 必做):
> ```
> ① 让球数 → 对主队生效还是对客队生效?
>    → 始终对主队。让-X: 主队扣球; 让+X: 主队加球
> ② 每个选项对应什么实况比分?
>    → 选3个典型比分逐项验证 (如: 1-0, 0-2, 0-3 各属哪项)
> ③ 赔率高/中/低 → 与实况概率高低是否一致?
>    → 如果赔率最低的选项和你认为"最可能"的实况不符 → 坐标系反了!
> ```
> **犯错历史**: 2026-06-26 把新西兰vs比利时 RQSPF 主客写反——误把 @2.05(比利时赢3+) 当作"赢1球/平/输"。原因是混用了"庄家亚盘(强队让球)"和"竞彩RQSPF(主队原点)"两套坐标系。

### Step 4 — 欧亚转换 + 陷阱
- **必须先读 KB-2 + KB-10 陷阱 #16-#21**
- 理论 AH vs 实际 AH 对比，15+6 条欧亚陷阱并行检测
- 输出：盘口偏差值 + 触发陷阱编号

### Step 5 — 开盘定位
- 开盘赔率 vs 公允价值，层级间对比
- 参考 KB-5
- 输出：`[场次] 开盘定位：[偏高/公允/偏低]，层级分歧：[是/否]`

### Step 6 — 临场变动
- 6h → 2h → 30min 水位趋势
- 回调验证 / AH 真假突破 / 初受盘差（KB-12）
- 输出：水位方向 + 突破真伪 + 初受盘差信号

### Step 7 — 6D 综合评分
- **必须先读 KB-4**
- 6 维度连续 0-6 分，维度去重叠
- 输出：每场 6 维得分 + 总分

### Step 8 — 陷阱全扫描
- **必须先读 KB-2 + KB-3 + KB-10**
- 49 条陷阱规则并行检测，按 **A/B/C 三层分类加权**（非简单计数）：
  - **C 类（信息型）**：赔率方向 vs 基本面不一致 → 真实警告，直接降信度
  - **B 类（诱导型）**：盘面刻意制造"危险信号" → 可能强化原方向（需成交量交叉验证）
  - **A 类（仓位型）**：机构被动调价，与基本面无关 → 降权/忽略
- 输出：触发清单 + 分类 + 综合判定（计数阈值已废弃，以分类加权为准）

### Step 9 — 方向判定 (v3.10.5: 共识方向 + 量化动机修正 + 平局分级 + 反投)

> **v3.9.0 更新**: 动机修正从 AI 主观定性改为量化算法。
> mot 值由 Step 2 输出的小组积分+ELO差距+赛程间隔自动计算 (KB-17.2.2)。
> 基线准确率: 54.7% (179场全量, 基于 Pinnacle 数据校准; 2022WC 验证 Pinnacle=市场均值)。分箱见 KB-16。

```
方向判定逻辑 (三步):

  1. 市场共识方向 = 30 家博彩公司收盘赔率平均值最低的那一方向 (H/D/A)
     计算: ouzhi 页底部"平均值"行的收盘赔率 (30家算术平均)
     基线准确率: 54.7% (179场全量, KB-16 分箱)

  2. 动机修正 (KB-17.2 量化算法):
     a. 判定轮次 (R1/R2/R3): Liansai API 同组比赛日期 → 推导
     b. 计算 mot: base_situation (小组积分榜查表) + elo_adjust + rest_adjust
     c. 执行五规则 (优先级: 规则4 > 规则2 > 规则1 > 规则3 — v3.10.7 规则2无赔率门槛+新增2b):
        规则4: 赔率<1.50 + R3 + 共识方向方mot≥0.85 → ✅ 生死战可信 (覆盖1/2)
        规则2: 共识方向方mot<0.25 → 按赔率分档 (见KB-17.2.4):
          2a (赔率<1.50) → 🚫 强制跳过 (可选反投)
          2b (赔率∈[1.50,2.00] + R3) → ⚠️ 降C类不入串关核心
        规则1: 赔率<1.50 + R1 → 🚫 跳过 (例外: 信号豁免→候选0.5x, 见 O-15)
        规则3: 赔率∈[1.80,2.20] + mot_gap<0.15 → ⚠️ 仅降仓0.5x

  3. 否定检查:
     a. KB-19偏离≥2条+动机冲突 → "跳过" (若对手赔率2-6→反投)
     b. KB-17平局二级信号≥3条 → "跳过"
     c. Kelly(共识方向)>1.05 → 仓位降0.5x
     d. 必发方向≠共识方向 → 仓位降0.5x
     e. KB-17平局一级信号触发1条 → 仓位降0.5x + 不入核心链接 (v3.8.1)
        触发2条一级信号 → 强制"跳过"
     f. R3 + 共识方向方mot<0.25 (规则2b触发) → 额外检查: 成交量信号降权50% (v3.10.7)
        逻辑: 锁定出线格局下必发成交量可能由散户资金主导(机构已平仓), 信号失真

反投规则 (KB-17b):
  动机共识冲突 + mot_gap>0.30 + 偏离≥2 + 对手赔率2.00~6.00
  → 反投单关¥10, 不进入串关。单日≤1场, 仅R3可用。
```

- 红 = 主胜 / 琥珀 = 跳过 / 绿 = 客胜 / 🔄 = 反投，结论置顶
- **必须输出信号来源分解面板**：
  ```
  ┌─ 方向判定 ─────────────────────────────────────────┐
  │ 市场共识方向: [H/D/A] (30家平均收盘赔率 X.XX最低) │
  │ 动机修正: [无/深盘慢热(R1)/动机流失(mot=X.XX)/    │
  │           均衡对耗/生死战(R3+mot=X.XX)]             │
  │ 偏离信号: [N/6条] → [正常/降仓/跳过/反投]         │
  │ 平局标记: [无/一级信号N条/二级信号N条→跳过]        │
  │ 最终方向: [H/跳过/A/🔄反投]                        │
  └──────────────────────────────────────────────────────┘
  ```
- 输出：`[场次] 方向：[胜/跳过/负/反投]，依据：共识方向+动机修正+否定检查`

### Step 10 — 比分预测引擎 (v3.8.0: OU+让球+Poisson量化)

> **v3.8.0**: 废除免责式"仅供参考"比分。替代为市场隐含Poisson+OU+让球三位一体引擎。
> 详见 knowledge-base.md KB-21(比分预测引擎) + KB-20a(信号5 OU+信号8 让球深度)。

```
四步计算 (KB-21):
  1. λ_total: 信号5 OU水位比 → 期望总进球
  2. λ_h/λ_a: 信号8 让球深度 → 非对称拆分
  3. Poisson概率矩阵 → Top N 比分
  4. (可选) 信号6 必发成交量 → λ校准

输出:
  比分预测 (带概率%):
    1. X-X (Y.Y%)  2. X-X (Y.Y%)  3. X-X (Y.Y%)
  期望进球: X.X球
  穿盘概率: XX% (让球深度≥1.5球时必输出，供 Step 11 穿盘风险检查使用)
  校准: WC48场 Top3=25.0% Exact=15.9%
```

### Step 10.5 — 信号可靠性检查 (KB-16 + KB-17, v3.7.0 校准)

> **v3.7.0**: OCI五分位体系废除。替代为两步检查:
>   (1) 共识方向可靠性: 偏离信号计数 (KB-19)
>   (2) 平局风险: 5条触发条件计数 (KB-17)
> 详见 knowledge-base.md KB-16 信号可靠性矩阵。

- **必须先读 KB-16 + KB-17 + KB-19**
- 输出：偏离信号数 + 平局触发数 → 方向/仓位调整

### Step 10.6 — 仓位 (见 betting-sop.md 1.2 三档仓位)

> **v3.10.2**: 废除旧版 核心/跳过/冷门翻转 三档。
> 替代为 betting-sop.md 1.2 基于竞彩 deVig 的三档仓位:
>   🔥 A类 (deVig≥0.65) → 重仓, 入串关核心
>   ➡️ B类 (deVig 0.55-0.65) → 中仓, 入串关非核心
>   ⚡ C类 (deVig<0.55+EV>0) → 不入串关, 仅轻仓单关
> **必须先读 betting-sop.md 1.2**

- 偏离检测（5条客观信号，见 KB-19）:
  - 升水>3% / 量价背离 / 市场分歧 / 平局>28% / OU下调>0.25
  - 偏离≥3 → 不入 A 类 (降为 B 类或跳过)
- 输出：每场档位归属 + deVig 值 + 偏离计数

### Step 11 — 概率驱动哑铃组合构建 + 仓位执行

> **v3.10.5 概率驱动哑铃组合**: 保本层预算由 P(≥2/3正确) 反推 (capped ¥40-75), 非固定金额。
>   🛡️ 保本层 (capped ¥40-75) — 高概率 3串3/2串1, 概率驱动分配
>   ⚡ 博上层 (capped ¥30) — 低概率正EV, 轻仓单关, 以小博大
>   💰 现金缓冲 (≥ ¥15) — 永不全押, 亏损永远有梯度

- **⚠️ 必须先读 `references/betting-sop.md` 完整五步流程**
- **⚠️ ⛔ 数据分界墙**: 投注只用竞彩赔率, Pinnacle/百家平均只用于分析层
- **⚠️ 竞彩可用性硬门 (1.1)**: SPF可用? RQSPF可用? 不可投直接剔除
- **⚠️ 哑铃组合 (1.5)**:
  - 🛡️ 保本层: deVig≥0.55, 3串3(¥75) 或 2串1(¥50), 2串1≥2.00
  - ⚡ 博上层: EV>0 + 赔率≥3.00, 单关 ¥10-15/注, ≤2注/日
  - 💰 现金缓冲: ¥10-20 永不动用
- **⚠️ 穿盘风险**: 让球深度≥1.5球执行 1.1b

### Step 12 — 报告生成
- **详见下方 Step 12 章节**
- HTML 输出：MBI 面板 + 6D 评分 + 比分 + 过关方案 + 免责声明
- 术语强制翻译（见 Output Format 术语表）

### 步骤完成自检（Step 12 前必须逐项打勾）

```
□ Step 0  match_list 已获取（含 shuju_id）| 逐场检查共享缓存后加载/抓取完成 | team_strength.json 已加载
□ Step 0a 基本面全量加载: ELO+积分榜(四验证+WebSearch)+形态H2H(shuju页)✅
□ Step 0.5 市场健康检查已完成（KB-14 → MPC/Veto/Breaker）
□ Step 1  队名已校验（KB-0），6 页齐全
□ Step 1.5 反叙事已输出 + 新闻搜索已执行（news-protocol.md 白名单）+ 新闻信号面板
□ Step 2  基本面量化六维度已输出（ELO+形态+H2H+伤病+赛程+形势+量化强度）
□ Step 3a MBI 面板已输出（6 模块全）
□ Step 3b Pinnacle 真实概率已输出
□ Step 4  盘口偏差+触发陷阱编号已输出
□ Step 5  开盘定位已输出
□ Step 6  水位方向+突破真伪已输出
□ Step 7  6D 每场得分已输出
□ Step 8  49 条陷阱全扫描+分类加权判定（A/B/C）已输出
□ Step 9  每场方向判定+信号来源分解已输出
□ Step 10 进球倾向（偏大/偏小/中性）已输出 + 参考比分（附免责）+ 穿盘概率（让球深度≥1.5球时）
□ Step 10.5 OCI客观指标+DRM平局因子已分析 → 输出综合信心分
□ Step 10.6 三档仓位+偏离检测已执行 → 输出档位归属 + 偏离计数 + 翻转方向
□ Step 11 betting-sop.md 已读 → 穿盘风险已检查 → 过关方案已输出（按¥100本金计算）
□ Step 12 报告已生成（亮色模式 + 比分附免责 + ¥明细 + 回报矩阵）
□ Step 13 已主动询问用户是否需要回测并处理用户答复
```

---

## ⚠️ 硬约束（全流程适用）

| 约束 | 触发条件 | 动作 |
|:---|:---|:---|
| 日熔断 | 当日累计亏损 ≥ 3% 总本金 | **立即停止**所有投注 |
| 周熔断 | 当周累计亏损 ≥ 8% 总本金 | **转入影子测试**（A类≥65%+反向≥70%+EV≥−3%） |
| 模块禁用 | 任一模块连续 7 次错判 | **禁用该模块**，不再参与评分 |
| 临场禁令 | 比赛开球前 ≤ 60 分钟 | **禁止生成新决策**，所有决策必须 >1h 前完成 |
| 倍投禁止 | 任何时候 | **绝对禁止**用加倍投注追损 |
| 对冲禁止 | 任何时候 | **绝对禁止**对同一比赛进行对冲投注 |

---

## 混合过关玩法速查

| 玩法 | 说明 | 关键约束 |
|:---|:---|:---|
| SPF 胜平负 | 猜胜/平/负 | 深盘场未开售 → 让球替代 |
| RSPF 让球胜平负 | 让球基础猜胜平负 | **⚠️ SPF≠RQSPF: "赢不赢"≠"赢几个", SPF结论不能直接当RQSPF结论 |
| JQS 总进球数 | 0/1/2/3/4/5/6/7+ | 每场最多选 1 个玩法 |
| BQC 半全场 | 半场+全场结果 | 同一场不能 SPF 和让球同时选 |
| BF 比分 | 猜具体比分 | 百家分析 → 竞彩投注 |

---

## Step 11: 量化投注 SOP

> ⚠️ **执行前必须先读** `references/betting-sop.md` **完整五步流程，不可仅凭下方速览执行。**
> 数学推导见 **KB-13** 赔率阈值数学逻辑 + 动态仓位分配规则。
> 赔率阈值表 + 三不选 + 拆分规则 + 滚动风控均在 betting-sop.md 中。

> **🚨 RQSPF市场分离铁律（P0 — 2026-06-26事故修复）**:
> SPF和RQSPF是两个独立的市场，SPF结论不能直接套用到RQSPF选项上。
> SPF答"赢不赢"，RQSPF答"赢几个"——两码事。
>
> ```
> ╔══════════════════════════════════════════════════════════════════╗
> ║  SPF可用 → 优先用SPF                                           ║
> ║  SPF不可用 → RQSPF替代 → 必须重新做"能赢几个"的独立分析          ║
> ║                                                                 ║
> ║  四步检查（缺一不可）:                                           ║
> ║  ① 标志位检查: SPF=不可用? → 进入RQSPF模式                       ║
> ║  ② 坐标映射: 让球数对谁生效? → 选3个典型比分验证3选项            ║  
> ║  ③ 实际表现检查: 强队前N场真实进攻数据(进几球/射正/机会转化)     ║
> ║  ④ 绿野仙踪问: "如果强队1-0领先，教练会让球员继续狂攻还是收缩?"   ║
> ║                                                                  ║
> ║  犯错历史: 2026-06-26 比利时vs新西兰                             ║
> ║   - 用SPF 1.20(赢球83%)直接匹配RQSPF 2.05(赢3+≈49%) → 错        ║
> ║   - 没有做"比利时1-0领先会怎么做" → 收缩保体力 → 选主胜@2.58      ║
> ║   - 没有查比利时前2场进攻数据(0胜2平仅1球) → 高估大胜概率        ║
> ╚══════════════════════════════════════════════════════════════════╝
> ```

> ⚠️ **必须先在betting-sop.md执行完上述四步检查后才进入SOP**

```
SOP 速览（每步详情见 betting-sop.md）:
  ① 信号筛选+赔率硬过滤 — 3串4: 2/3个2串1≥1.65 + 3串1≥4.50 / 三不选 / 滑点复验（详见betting-sop.md 1.5阈值速查表）
  ② 物理拆分+独立定价 — 禁止终端机套餐，每注独立计算 EV
  ③ EV 计算+非对称注码 — 剔除EV<−5% → 防守底线 → EV加权 → 风险平价
  ④ 账户状态微调 — 顺风放利润 ≤30% / 逆风砍进攻 ≤0%
  ⑤ 滚动风控+独立复盘 — 时间差止损 / 逐注拆解验证

输出: 过关方案 + 仓位分配 + 回报矩阵
```

---

## Step 12: 报告生成

### 12.1 输出前检查
```
□ 所有术语已翻译为买菜大妈能懂的中文
□ 术语翻译已检查（所有英文缩写按翻译表转大白话）
□ 免责声明已包含（含比分预测免责）
□ 时间戳标注为北京时间
□ 引擎版本号 v3.8.1 + 数据截止时间已标注
□ 使用亮色模式（浅色背景，深色文字）
□ 参考比分已附带"仅供娱乐参考"
□ 所有金额为 ¥ 展示，默认本金 ¥100
□ 【新】所有投注建议基于主队视角: 方向描述用"主队赢/主队平/主队输"，赔率用"主队/平局/客队"
□ 【新】单关建议已核验: 仅推荐出现在 g=1 页面的比赛的单关方案
```

### 12.2 HTML 报告结构
```
assets/report-template.html 作为基础模板，注入以下模块：
  1. 今日总览（方向 + 风险等级 + OCI 综合信心 + 推荐串关）
  2. 每场分析（MBI 面板人话 + 信号来源分解 + OCI 客观指标 + 偏离信号 + 基本面 + 进球倾向 + 陷阱 + 参考比分）
  3. 竞彩建议（M串N 方案 + ¥ 金额明细 + 回报矩阵 + 四种情景表）
  4. 风险提示（熔断状态 + 风险日级别）
  5. 报告元信息（时间/版本/数据截止）
  6. 免责声明（教育用途 / 不构成投注建议 / 比分仅供参考）
```

### 12.3 输出后验证
```
□ 亮色模式已应用（浅背景 + 深色文字）
□ 每场参考比分已包含（附带"仅供参考"声明）
□ 报告中无任何英文术语或内部缩写
□ 所有赔率均为竞彩真实赔率（非 Pinnacle/bet365）— 须验证数据来自 rangqiu 页面的"竞*官*"行，而非百家平均
□ trade.500.com 首页赔率已排除（首页显示的是百家平均，不是竞彩赔率）
□ 所有金额基于 ¥100 本金计算，展示 ¥ 明细
□ OCI 指标权重和偏离计数已在每场报告中标注
□ 已加入免责声明（含比分预测免责）
□ **【主队视角检查】所有投注建议以主队为参照系**:
   - "主队赢"而非"客队输"
   - "主队让球胜/平/负"而非"客队+1"
   - 赔率列名 = "主队/平局/客队"
   - 让球方向: "主队让1球"而非"客队受让1球"
□ **【单关检查】仅保留出现在 g=1 页面的单关建议**
□ **【RQSPF坐标系验证】如有RQSPF选择，必须做三坐标映射验证**:
   - 让球数对主队生效 → 选3个典型比分验证3选项归属
   - 赔率高低与实况概率方向一致? 赔率最低的应是最可能结果
   - SPF结论不能直接当RQSPF结论——"赢不赢"≠"赢几个"

---

## Step 13 — 赛后回测触发器

> 报告输出完成后必须执行以下步骤。

### 13.1 主动询问用户

```
输出完 Step 12 的最终报告后，必须主动问用户：

"是否需要对这些比赛的回测？如果选择了投注，您想知道盈亏情况分析吗？"

询问语气要自然，不要机械。参考话术：
  "今天的分析结果如上。要不要跑一下回测，看看这些比赛实际踢完会是什么结果？"
  "以上是今天的投注建议。比赛结束后如果想做系统复盘，我可以启动回测工作流。"
```

### 13.2 触发回测工作流

如果用户确认需要回测，按以下流程：

```
1. 调用 Skill → football-backtest-workflow（位于本技能目录下）
2. 将当前分析的数据日期 + 分析结果作为输入传给回测工作流
3. 回测工作流会完成：盈亏计算 → 多假设检验 → 信号拆解 → 反事实推演 → 决策审计 → 根因决策树 → 时间旅行验证 → 9 层报告
4. 最终输出完整的回测复盘报告
```

> ⚠️ **注意**: 回测依赖 `football-backtest-workflow` 技能，该技能位于本技能目录的 `football-backtest-workflow/` 子文件夹中。
> 如果用户不确认回测，或者比赛尚未结束，尊重用户选择，不强行启动。

### 13.3 自检

```
□ 报告已输出给用户
□ 已主动询问是否需要回测
□ 用户答复后已按对应流程处理
□ 如启动回测 → 完整输出 9 层复盘报告
□ 如不回测 → 告知用户"需要时随时可以跑回测"
```

---

## Output Format — 大白话输出

**核心原则**：最终给用户的报告里，所有英文缩写和专业术语都要翻译成买菜大妈都能看懂的大白话。庄家名字（Pinnacle等）保留原名。

**⚡ 投注建议必须基于主队视角书写（铁律）**:
- 方向描述一律用"主队赢/主队平/主队输"，不能用"客队输/客队赢"
- 让球方向一律用"主队让X球/主队受让X球"，不能用"客队+1球"
- 赔率列名 = "主队/平局/客队"（不是"胜/平/负"，也不是"客队/平/主队"）
- 大小球始终用"进球多/进球少"描述
- **反例**："客队+1球负" → 应写成"主队让1球负"
- **正例**："主队让1球胜(1.92) / 主队让1球平(3.25) / 主队让1球负(3.11)"

### 术语翻译对照

| 内部术语 | 大白话翻译 |
|:---|:---|
| Pinnacle | Pinnacle（保留原名，行业最精明的博彩公司） |
| 百家平均赔率 | 30家博彩公司的平均价 |
| OCI-1 赔率变轨 | Pinnacle 怎么调价的 |
| OCI-2 市场共识 | 30家公司是不是跟 Pinnacle 一个看法 |
| OCI-3 成交量 | 实际下注的钱流向了哪边 |
| OCI-4 离散度 | 30家公司意见是统一了还是吵起来了 |
| OCI-5 亚盘验证 | 让球盘的走势和胜平负是不是一致 |
| SPF | 胜平负 |
| AH/让球盘/亚盘 | 让球盘（强队让弱队几个球） |
| OU/大小球 | 总进球数大小 |
| 水位 | 赔率的水分/利润率 |
| 初盘 | 开盘时的赔率 |
| 终盘/临场 | 开赛前的最终赔率 |
| 升水 | Pinnacle 调高了对某方的赔率（不看好） |
| 降水 | Pinnacle 调低了赔率（看好） |
| 离散收敛 | 30家公司的意见趋于一致 |
| 离散发散 | 30家公司吵起来了，分歧加大 |
| 核心仓位 | 最看好的投注，可以放心买 |
| 跳过/不投 | 没把握，今天不买 |
| 冷门翻转 | 系统觉得可能要爆冷，建议反着买 |
| 偏离信号 | 市场出现异常的警告信号 |

### 报告结构（给用户看的）

```
## 今天怎么看

[一句话总结：今天看好哪些队 + 整体风险]

## 每场比赛

### 编号 | 主队 vs 客队 | 几点踢

> 胜负方向: 主队赢/平局/客队赢（把握: 高/中/低）  ← 以主队为参照
> 进球多少: 进球多/进球少/不好说（把握: 高/中/低）
> 可能比分: X-X / X-X / X-X（仅供娱乐参考）
> 让球方向: 能赢盘/赢不了 | 大小球: 大球/小球

两队近况: [2-3句，不说术语]
庄家怎么看: [翻译成：Pinnacle 觉得... 其他公司觉得...]
信号来源: [赔率自己说的 / 赔率变动告诉我们的 / 下注资金流向 / 其他信息]
综合判断: [🔥很有把握/⏭️没把握跳过/🔄可能要爆冷] — 异常信号: [几个]
风险提示: [有什么坑，白话解释]

## 竞彩怎么买

主推方案 / 怎么组合 / 赢了输了多少 / 几种可能情况
（所有钱按 ¥100 算，显示 ¥ 多少钱）

## 风险提示

（哪天该停手 / 报告生成时间 / 引擎版本 / 数据截止时间）

## 免责声明

仅供学习参考，不构成投注建议。比分预测纯属娱乐。
```

### 术语翻译对照（续）— 常用缩写用大白话

报告中遇到这些术语和缩写时，按以下翻译对照写成大白话：

| 缩写/术语 | 报告中写成 |
|:---|:---|
| MBI | 多家博彩公司综合信号 |
| OCI | Pinnacle 的赔率变动方向 |
| SPF | 胜平负 |
| AH / 让球盘 | 让球盘（强队让弱队几个球） |
| OU / 大小球 | 总进球数多少（大球/小球） |
| EV | 预期回报 |
| ROI | 收益率 |
| Kelly | 仓位比例公式 |
| Shin | 去抽水算法 |
| 离散度 | 30家公司的意见分歧 |
| 水位 | 回报率 |
| 抽水 | 博彩公司抽走的利润 |
| 初盘 | 开盘时的赔率 |
| 终盘/临场 | 开赛前的最终赔率 |
| 核心仓位 | 很有把握，可以买 |
| 跳过/不投 | 没把握，今天不买 |
| Pinnacle | Pinnacle（保留原名） |
| bet365 / 威廉希尔 | 保留原名 |

---

## KB Index（21 模块） — 文件分责

| KB | 内容 | 文件 | 对应步骤 |
|:--:|------|:---:|:--:|
| 0-8 | 盘口分析基础（队名/数学/陷阱/6D/基本面/logit/进球倾向/方法） | `knowledge-base.md` | 1-10 |
| **9** | **复盘/回测 → 用 football-backtest-workflow** | **`football-backtest-workflow/`** | **赛后** |
| 10-15 | MBI/数据校准/高级信号/风控/庄家读心/外源信号 | `knowledge-base.md` | Pre-1~11 |
| **16** | **Pin分箱准确率 (179场)** | **`knowledge-base.md`** | **2** |
| **17** | **平局专项+量化动机修正** | **`knowledge-base.md`** | **9** |
| **17b** | **mot计算+反投框架** | **`knowledge-base.md`** | **9** |
| **18** | **仓位分配 → 见 betting-sop.md** | **`betting-sop.md`** | **Step 11** |
| **19** | **偏离检测** | **`knowledge-base.md`** | **10.6** |
| **20** | **回测验证 → 用 football-backtest-workflow** | **`football-backtest-workflow/`** | **赛后** |
| **20a** | **11信号引擎** | **`knowledge-base.md`** | **3-10** |
| **21** | **比分预测引擎 (Poisson)** | **`knowledge-base.md`** | **10** |
| **SOP** | **量化投注 SOP + 风控细则** | **`betting-sop.md`** | **Step 11** |
| **FUND** | **基本面数据模块 (6协议文件)** | **`references/fundamentals/`** | **0a, 2** |

---

## Boundaries & Iron Rules

- 教育用途。数据源：500.com（30 家，零配额）。xG 为市场隐含。
- **玩法铁律**：
  - ❌ 禁止 N串1 → ✅ 强制 M串N（3串4 / 4串11）
  - ❌ 禁止均注 → ✅ 物理拆分 + 四步动态决策树
  - ❌ 禁止同联赛/同时间/同战意入同一串关（三不选）
  - ❌ 禁止 C 类信号入串关（仅可极轻仓单关，cap: 1.5% 本金）
  - ❌ 禁止终端机套餐 → ✅ 每一注独立下单
  - ✅ 赔率底线: 3串4 2/3个2串1≥1.65 + 3串1≥4.50 / 4串11 4/6个2串1≥1.65 / 2串1回退 ≥1.80（详见betting-sop.md 1.5阈值速查表）
  - ✅ 每日最多 1 组 M串N | 滑点 3.5% 前置 | 防守底线 > 80% → 放弃
  - ✅ 空仓即赢：找不到正 EV or 防守底线算不过 → 跳过当日
  - ✅ 保本层预算: P(≥2/3正确) 反推 (capped ¥40-75), 非固定金额
- **报告规格**：
  - ✅ **亮色模式**：浅色背景（#f5f7fa / #ffffff），深色文字（#1a1a2e / #333），所有卡片用浅底+深色边框
  - ✅ **参考比分**：每场输出 Top 3 可能比分，必须附带"仅供参考，不构成投注依据"
  - ✅ **默认本金 ¥100**：所有仓位按 ¥100 总本金计算，展示 ¥ 明细
  - ✅ **主队视角**：所有投注建议的参照系 = 主队（方向、让球、赔率列名全部从主队出发）
- **数据源铁律**：
  - ✅ **分析数据源**: 全球数据 (Pinnacle + 30家博彩公司) — ouzhi/yazhi/shuju/touzhu 页面
  - ✅ **投注数据源**: 竞彩官方赔率 — rangqiu 页"竞*官*"行 + trade 页 SPF 列
  - ❌ **禁止混用**: Pinnacle 赔率不能用于竞彩 EV 计算，百家平均不能充当竞彩赔率
- **熔断**：日亏 3% → 停 / 周亏 8% → 影子测试 / 模块 7 连错 → 禁用
- **影子测试**：A类 ≥ 65% + 反向 ≥ 70% + EV ≥ −3% 方可实盘
- **临场 30min 禁令**：所有决策 >1h 前完成 | ❌ 倍投 | ❌ 对冲

## 赛后复盘

回测复盘统一使用 `football-backtest-workflow/` 子技能。详见 Step 13 赛后回测触发器。

### Changelog

- **v3.10.8**: **RQSPF市场分离检查 + 三个交叉验证铁律**（2026-06-26）: 基于2026-06-26比利时vs新西兰场RQSPF方向判断失误修复. (1) **P0-1 Step 11前置检查**: 新增RQSPF市场分离铁律,P0级检查框 — SPF≠RQSPF,"赢不赢"≠"赢几个",SPF结论禁止平移至RQSPF (2) **P0-2 三步验证**: Step A(SPF可用性判定) + Step B(三坐标映射验证—3个典型比分) + Step C(实际表现检查—进攻数据+FIFA排名幻觉+教练策略推理) (3) **P0-3 betting-sop.md 1.1b重写**: 加入完整的市场分离检查+三步验证流程, 执行顺序: 市场分离检查→穿盘风险检查→纳入候选池 (4) **P1-1 输出验证新增**: Step 12.3加入RQSPF坐标系验证勾选项 (5) **P1-2 混合玩法速查**: RQSPF行加入"SPF结论不可平移"约束标注 (6) 版本号3.10.7→3.10.8

- **v3.10.7**: **规则2无赔率门槛 + 伤病logit按角色分层**（2026-06-26）: 基于2026WC D/E/F末轮回测. (1) **P0-1 KB-17.2.4 规则2重构**: 取消<1.50赔率限制, 按赔率分两档处理 — 2a(<1.50→跳过) + 2b(1.50-2.00+R3→C类不入串关核心); 修复美国(1.89)/德国(1.63)两场锁定出线漏检bug (2) **P0-2 伤病logit按角色分层**: news-protocol.md + SKILL.md Step 2 从统一-0.15改为持球核心-0.25/射手+防线-0.20/角色球员-0.10/双核-0.35 (3) **否定检查新增f条**: 规则2b触发时成交量信号降权50% (4) **四规则→五规则**: KB-17.2.4 标题更新 (5) 同步更新 betting-sop.md 1.2a 轮换风险分支

- **v3.10.5**: **全技能14项冲突修复**（2026-06-25）: 基于跨文件审计报告修复 14 项冲突。(1) **P0-1 竞彩官网数据源落地**: sporttery.cn 页面抓取脚本 fetch_sporttery.py 创建; scraper 从 500.com-only → sporttery.cn主源+500.com后备; betting-sop.md 1.1 重写为页面抓取优先 (2) **P0-2 仓位系统统一**: betting-sop.md 附录旧三档(核心/跳过/冷门翻转)删除, 全系统统一为 A/B/C deVig (3) **P0-3 KB-5b 清理**: 删除 OCI权重表+所有[已废除]标记, 保留平局信号强度分级表, 指向 KB-17.3 (4) **P0-4 betting-sop 1.1b补写**: 深盘穿盘风险检查(亚盘交叉验证+三级响应), 区分 RQSPF≠SPF (5) **P1-1 保本层概率驱动**: 固定金额→P(≥2/3正确)反推, capped ¥40-75 (6) **P1-4 Kelly公式写入 betting-sop.md** (7) **P2-1 流程图更新**为哑铃组合 (8) **P2-2 KB-20a 信号数修正** 10→11 (9) **P2-4 步骤数统一** (10) 所有过期内容直接删除不留标记

- **v3.10.4**: **全技能审计修复**（2026-06-25）: (1) 修复 21 个跨文件不一致 — 版本号统一/H1标题更新/KB索引补全/废除公式标注/重复标题合并/Step10.6冲突清理/fundamentals README补全/Step1数据路径更新/保本层金额一致化/KB-20a信号数修正 (2) sporttery-protocol v2.0→v2.1 双通道接入 (MCP+直连API, 四重降级) (3) 30min禁令统一为>1h前完成 (4) 版本 3.10.1→3.10.4 (含审计期间中间版本 3.10.2/3.10.3)

- **v3.10.3**: **哑铃组合 + 亏损梯度**（2026-06-25）: (1) 保本层¥50-75 (3串3) + 博上层¥20-30 (单关¥10-15) + 现金缓冲¥10-20 (2) 5种亏损情景枚举 (3) 1.5筛选链重写为哑铃两层

- **v3.10.2**: **数据分界墙+竞彩硬约束**（2026-06-25）: (1) 分析→执行仅传递4项 (2) 三档仓位改为竞彩deVig (3) 1.1竞彩可用性强化为P0硬门

- **v3.10.1**: **保守优先投注策略**（2026-06-25）: (1) betting-sop.md 1.5 完全重写为保守优先筛选链 — 核心指标从"最大化回报"切换为"最大化不亏钱概率P(≥2正确)" (2) 新增 1.5a 入串概率门槛: 每腿 deVig≥0.55 才能入串, Pin方向概率<55%→不入串 (3) 1.5b 默认载体从 3串4→3串3(¥75) (4) 1.5c 回血门槛从 2串1≥1.65→2.00 (5) 版本 3.10.0→3.10.1

- **v3.10.0**: **EV驱动串关筛选+竞彩官网数据源**（2026-06-25）: (1) 废除 3串1≥4.50/4串1≥8.00 任意赔率阈值, 替代为 EV 驱动筛选 — 3串1腿 EV = P_hit×payout−1, EV<−0.05→裁剪 (2) 新增 fundamentals/sporttery-protocol.md — 竞彩官网 (webapi.sporttery.cn) 数据源协议, 含 SportteryAPI MCP 部署指南 (3) KB-13.8b EV公式更新: 从 Pinnacle_deVigProb 切换到竞彩自身去水概率 (竞彩市场自洽) (4) betting-sop.md 1.5 完全重写为 1.5a(2串1底线)+1.5b(EV筛选)+1.5c(竞彩EV公式)+1.5d(阈值表) (5) 2串1 1.65 底线保留 (数学推导, 非任意) (6) 版本 3.9.4→3.10.0

- **v3.9.3**: **积分榜通用化+双源交叉验证**（2026-06-25）: (1) standings-protocol.md v1.0→v2.0 — 适用范围扩展至所有含小组赛赛事 + WebSearch交叉验证 (2) URL通用化: zuqiu-{sid}/jifen-{jid}/ (3) 三重保障: 主源+数学自检+搜索对照

- **v3.9.2**: **积分榜获取协议 — 修复AI编造积分致命bug**（2026-06-25）: (1) 新增 standings-protocol.md (2) Step 0a 积分榜强制抓取 (3) 德国6分被错报为3分的bug修复

- **v3.9.0**: **基本面量化+新闻协议**（2026-06-25）: (1) 新增 references/fundamentals/ 模块 — team_strength.json (ELO/FIFA/实力差/64队数据) + news-protocol.md (白名单/黑名单/交叉验证/3条量化规则) + README.md (模块说明) (2) SKILL.md Step 0a 新增基本面数据加载 (3) Step 1.5 新增新闻搜索 (伤病/首发/天气/突发事件, 白名单限定, 赔率时间戳交叉验证, 新闻只降信心不翻方向) (4) Step 2 从纯人工定性重构为 ELO 量化四维度 (实力对比/伤病logit/赛程体能/小组形势) (5) 版本号 3.8.2→3.9.0

- **v3.8.2**: **深盘穿盘保护+动机流失精炼**（2026-06-25 回测驱动）: (1) betting-sop.md 新增1.1b深盘穿盘风险前置检查 — 让球深度≥1.5球时读取Poisson穿盘概率, <60%剔除让球胜, <50%剔除该场 (来源: 摩洛哥让-2球赢4:2但穿盘失败, 赛前Poisson预测穿盘概率仅~55%) (2) betting-sop.md 1.2a新增动机流失精炼检查 — 赔率大涨>20%时增加竞彩-Pinnacle价差+让球深度二层验证, 防止小组末轮过度反应 (来源: 瑞士赔率涨31%被跳过但实际2:1胜, 竞彩赔率2.21远低于Pinnacle2.53) (3) SKILL.md Step 10 穿盘概率成为必输出字段, Step 11 新增穿盘风险前置检查引用 (4) knowledge-base.md KB-17.5 新增3条观察项: O-15(死亡区间信号豁免), O-16(动机流失价差验证), O-17(穿盘概率<60%阈值) — 均为n=1, 待跨赛事验证 (5) 版本号 3.8.1→3.8.2

- **v3.8.1**: **平局信号分级+胆排除+离散绝对值**（2026-06-25 回测#20260624驱动）: (1) KB-17.3 平局信号从等权5条计数改为一级/二级分级体系 — 新增第0条一级信号: Pinnacle平赔降幅≥20%单条触发降仓(来源: 英格兰0-0加纳平赔-33%被原阈值3.20过滤) (2) SKILL.md Step 9 否定检查新增e条: 一级平局信号触发→降仓0.5x+不入核心链接 (3) betting-sop.md 胆识别新增2条排除条件: 平赔降幅≥20%自动降为候选 + 离散度绝对值>25自动降为候选 (4) KB-19.1 偏离信号新增第7条: 离散度绝对值>25作为独立偏离信号(与上升>20%独立计数)

- **v3.7.0**: **Pin方向单一判定+信号精简**（2026-06-24 WC44场校准驱动）: (1) Step 9 废除 MBI×0.6+OCI×0.3+traps×0.1 复合公式 — 44场数据证实该公式比单纯Pin方向低16.4pp (2) 方向判定改为Pin收盘最低赔率方向+否定检查(偏离≥3条→跳过/平局≥3条→跳过/Kelly>1.05降仓) (3) knowledge-base.md KB-16 重写为信号可靠性矩阵(Kelly 43.2%/OCI-1 38.6%/Prob 31.8%确认为反向指标并删除) (4) KB-17 废除DRM打分,改为平局5条触发计数 (5) KB-19 偏离从冷门翻转改为否定信号 (6) Step 10.5-10.6 精简为两步检查 (7) 删除OCI五分位体系和179场权重查表

- **v3.6.1**: **3串4成为主力载体**：(1) betting-sop.md 核心原则改为3串4默认，新增第二步-A注码分配模板（3串4/4串11/2场日回退三张注码表）(2) 赔率硬过滤阈值重写为3串4语境（2/3个2串1≥1.65）(3) 四步决策树改写为3串4 ¥100预算版本 (4) SKILL.md 本金约定/阈值引用同步更新 (5) 数据支撑: 44场回测 3串4 ROI +21.0% vs 2串1 -9.5%

- **v3.6.0**: **删除旧回测模块，统一用新技能**：(1) 删除 references/postmortem.md（旧 O-1~O-14 回测观察）(2) analyst SKILL.md 删除所有 postmortem.md 引用 (3) knowledge-base.md/betting-sop.md 同步更新为指向 football-backtest-workflow/ 子技能 (4) 赛后复盘统一由 Step 13 触发 football-backtest-workflow

- **v3.5.9**: **全技能输出大白话化**：(1) backtest-workflow 新增完整 Output Language 章节（术语翻译对照表/每层语言检查） (2) scraper v2.5 新增 JSON 字段中文友好规范 (3) 庄家名（Pinnacle/ bet365等）保留原名不翻译 (4) 所有英文缩写按翻译表转大白话
- **v3.5.7**: **共享缓存协议**：(1) 所有足彩技能共用 .cache/shared-football/ 缓存目录 (2) scraper v2.3 新增 Phase 0 Shared Cache Protocol (3) analyst/backtest 同步更新
- **v3.5.6**: **新增赛后回测触发器**：(1) 新增 Step 13 赛后回测触发器——报告输出后主动询问用户是否需回测，确认即启动 football-backtest-workflow (2) 执行纪律更新为"14步"(13步+Step 13) (3) 步骤自检加入 Step 13 选项 (4) 新增 Step 13 章节含询问话术+触发流程+自检清单

- **v3.5.5**: **数据源分离+单关检查+主队视角**：(1) 明确分层: 分析用全球数据(Pinnacle+30家), 投注用竞彩官方赔率(rangqiu页"竞*官*"行), 严禁混用 (2) Step 11 新增单关可用性核验, 参考 trade.500.com/jczq/?playid=312&g=1 页面 (3) Output Format 新增主队视角铁律, 所有投注建议以主队为参照系 (4) 更新12.1/12.3 自检清单加入主队视角和单关检查项 (5) 500com-football-scraper 同步更新: Phase 1 新增 g=1 单关页面抓取, rangqiu 解析章节新增"竞*官*"行提取说明 (6) 更新 Boundaries & Iron Rules 加入数据源和主队视角铁律

- **v3.5.4**：**竞彩赔率数据源固化**：(1) Step 0 新增竞彩赔率提取规范——必须从 rangqiu 页面的"竞*官*"行获取，严禁使用 trade.500.com 首页百家平均数据 (2) 输出验证新增竞彩数据源核查项
- **v3.5.3**：**2026世界杯驱动仓位重构 — 模式驱动核心仓位**：(1) CORE增加模式匹配路径: OCI4=CONVERGE+OCI1≠RISE+OCI5=SYNC (2026WC验证: 5/5=100%) (2) OCI4=DIVERGE禁止进入CORE (12场中6/6=50%=噪声) (3) 信心分阈值从0.12降至0.08 (4) 核心仓位路径优先级: 模式匹配>信心分数 (5) 废旧版核心规则(0.12+≥3正向)
- **v3.5.2**：**179场回测驱动仓位重构**：(1) 砍掉STANDARD仓位(179场仅51%命中率=抛硬币), 仓位简化为三档(核心/跳过/冷门翻转) (2) OCI权重归一化为(命中率-0.50)×2, 加入16.4节实测权重表 (3) OCI1_RISE(46.9%)和OCI4_NEUTRAL(42.9%)确认为持续低于随机的信号, 权重为负 (4) OCI2_AGREE(54.8%)降为弱正向(权重+0.10) (5) 冷门翻转触发条件从严: 偏离信号≥2→≥3 (6) 核心仓位门槛从≥65%改为信心分≥0.12+≥3正向OCI
- **v3.5.1**：**四文件分责清理 + 统计审计**：(1) 四文件严格分责: knowledge-base(盘口分析)/betting-sop(投注执行)/postmortem(复盘回测) (2) 撤回4条拍脑袋结论(统计显著性不足) (3) 新增OCI4_CONVERGE(59.7%, p=0.034)权重提升 (4) KB-18(仓位)+KB-13.2(信号分级)+KB-13.8b(EV阈值)→移入betting-sop (5) KB-20(回测验证)→移入postmortem
- **v3.5.0**：**客观指标体系重构**：(1) KB-16完全重写: 13条静态FVS规则替换为5个OCI客观变化指标 (2) KB-17清理: 废除DRM-7/9/10, DRM降为OCI辅助因子 (3) KB-18新增第四档(冷门翻转) (4) KB-19重写: 废除赛事感知, 改为5条客观偏离信号 (5) KB-20新增OCI模式库查表 (6) 核心原则: 全部基于盘口客观变化, 排除主观标签
- **v3.4.0**：**跨赛事回测驱动重构**：(1) **三档仓位引擎**：KB-18, 核心78%/标准52%/不投区, 仓位乘数化 (2) **FVS-1A清零**: FVS=1→0（三个赛事42%反向指标）(3) **DRM-7降权**: +2→+1, 不再自动触发VETO, 新增DRM-9/10 (4) **边缘探测器**: KB-19, Pinnacle开盘偏差+量价背离+赛事类型感知 (5) **回测验证框架**: KB-20, 三关验证流程 (6) 跨赛事数据: wc2018_backtest.json(64场) + euro2024_backtest.json(51场) (7) 版本v3.3.2→v3.4.0
- **v3.3.2**：**用户配置固化**：(1) Step 10 恢复参考比分输出（附带免责）(2) Step 11 默认本金 ¥100 计算仓位 (3) Step 12 报告为亮色模式（浅底深字） (4) 更新自检清单和输出验证项
- **v3.3.1**：**回测校准**：(1) DRM≥2 升级为 🔴VETO（2022WC回测: 命中率63%→22%, 净收益+9）(2) FVS=3+DRM≥1 升级为VETO (3) FVS×DRM矩阵从16格简化为10格 (4) 整体命中率 53.1%→65.9%（+12.8pp）
- **v3.3.0**：**双引擎+双辅助架构**：(1) KB-16 FVS 热门脆弱性引擎 (2) KB-17 DRM 平局风险模块+FVS×DRM叠加矩阵 (3) KB-6+KB-10.8 统一logit管线+四组降权+总量cap (4) KB-13.8 竞彩市场转换层 (5) Step 10.5 FVS + Step 10.6 DRM + Step 11 竞彩EV前置检查
- **v3.2.0**：**三层架构改造**：(1) 砍除 Top 3 比分精确输出，改为进球倾向三档判定 (2) 49 条陷阱从计数改为 A/B/C 三层分类加权 (3) MBI 输出加入信号来源分解面板 (4) Lead-Lag + Exchange 新增量价交叉验证四象限矩阵 (5) 新增 KB-15 外源信号注入路线图
- **v3.1.3**：新增 post-mortem 观察项 O-6 至 O-10
- **v3.1.2**：SOP 重构为五步标准作业程序
- **v3.1.1**：仓位分配升级为四步动态决策引擎
- **v3.1.0**：5 票金字塔级联 → M串N 容错 + 非对称注码
- 旧版逻辑归档：`旧版投注逻辑_v3.0.5_金字塔级联与枚举最优票集.md`
