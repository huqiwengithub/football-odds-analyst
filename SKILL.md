...
First time a league/tournament is used:
  ...
  2. If NOT:
     ...
     → 1 quota → save response to ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_fixtures_{tournamentId}.json
...
```
Cache file: ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_fixtures_16.json
...
Outcome ID cache: ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_outcome_ids_{tournamentId}.json (全赛事，3 家博彩商)

首次运行（全赛事一次，3 配额 — 每家博彩商 1 次）:
  ① 🔴 ask user: "/v4/odds-by-tournaments × 3 = 3 配额。是否继续？"
  ② 串行拉取 3 家博彩商的 outcome ID（每家 ~2.4MB, ~20s）:
     GET /v4/odds-by-tournaments?bookmaker=pinnacle&tournamentIds={tournamentId}&apiKey=KEY
     Wait ≥1s (rate limit 500ms for this endpoint)
     ...
     → 为每个 fixtureId × 每个 bookmaker 提取:
       a. AH main: "line/.../spreads" → mainLine=true 的 outcome IDs
       b. OU main: "line/.../totals"   → mainLine=true 的 outcome IDs
       c. CS: "correct_score" (if available) → all outcome IDs
       d. TG: "exact_goals" (if available) → all outcome IDs
     → 保存: ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_outcome_ids_{tournamentId}.json

后续所有查询（0 配额，秒级）:
  ③ 读缓存 → 获取目标 fixture 的 3 家 outcome ID
  ④ Call A: /v4/historical-odds?outcomeId=101,102,103 (1X2, 3家, ~510KB)
  ⑤ Wait ≥5s
  ⑥ Call B: /v4/historical-odds?outcomeId={cached_AH_OU_CS_TG_3家} (3家, ~600KB)
     → AH/OU daily sampling, CS/TG latest snapshot → ~12KB per match

全赛事 3 配额(一次性) | 永久 0 | 三家 AH/OU 全覆盖欧亚对照+冷热差
```
...
> - Call B: `outcomeId={cached from first /v4/odds call}` → AH + O/U + CS + TG（3家, ~600KB）
...
Phase 0 ─ One-time initialization (⚠️ BILLED — MUST confirm with user first)
  ...
  → save to ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_fixtures_16.json
  → 1 quota (once only, never refetch — subsequent reads from cache are 0 quota)
  → future analyses read from cache at 0 quota

Phase 0 ─ One-time outcome ID cache (⚠️ 3 quota, user confirmed, 3×/v4/odds-by-tournaments)
...
─────────────────────────────────
fixtures cache(1) + outcome IDs cache(3) = 4 (全赛事一次性)
─────────────────────────────────
后续所有分析: 0 | 总计: 4 / 250
```
...
```
Phase 0: ⚠️ /v4/fixtures × 1 + ⚠️ /v4/odds-by-tournaments × 3 = 4 quota (一次性)
...
2. Check ~/.workbuddy/skills/football-odds-analyst/.cache/oddspapi_fixtures_16.json → exists → read from cache (0 quota)
